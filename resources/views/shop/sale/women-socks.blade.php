...
<div class="bg-[#f5f2ed] min-h-screen">
    ...
    <section class="relative w-full h-[50vh] md:h-[60vh] bg-cover bg-center rounded-2xl" 
             style="background-image: url('/images/sale-socks-hero-women.webp');">
    ...
    <div class="w-full max-w-6xl mx-auto px-6 py-4 flex items-center justify-between bg-[#E6DDD0] rounded-[40px] mt-4">
        ...
        <div class="flex items-center gap-4">
            ...
            @php
                $saleCount = \App\Models\Product::where('type', 'socks')
                                                ->where('gender', 'women')
                                                ->where('on_sale', true)
                                                ->count();
            @endphp
        ...
        <div class="flex items-center gap-4">
            ...
            <div class="flex items-center bg-[#E5DFD6] border border-[#D4CDC3] rounded-full p-[4px]">
                <a href="{{ url('/sale/men/socks') }}" class="px-6 py-1.5 rounded-full text-[13px] font-medium transition text-black">
                    MEN
                </a>
                <a href="{{ url('/sale/women/socks') }}" class="px-6 py-1.5 rounded-full text-[13px] font-medium transition bg-black text-white">
                    WOMEN
                </a>
            </div>
            ...
            <div class="relative inline-block">
                ...
                <div id="dropdownMenu" class="absolute right-0 mt-2 w-44 bg-black border border-blue-700 rounded-xl shadow-lg hidden z-30">
    ...
    <section class="max-w-full px-4 py-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-6">
        @php
            $products = \App\Models\Product::where('type', 'socks')
                                          ->where('gender', 'women')
                                          ->where('on_sale', true)
        ...
        <div class="relative group w-full block bg-white p-6 pt-10 h-[420px] rounded-2xl shadow-2xl overflow-hidden cursor-pointer transition-all duration-300 ease-out hover:h-[520px] hover:-translate-y-2">
            <span class="absolute top-4 left-4 bg-red-500 text-white text-xs px-3 py-1 rounded-full z-10 font-semibold">SALE</span>
            <img src="{{ asset('storage/' . $product->image) }}" class="w-full h-48 object-cover rounded-xl" alt="{{ $product->name }}" />
            <h4 class="mt-20 text-sm font-semibold text-black">{{ strtoupper($product->name) }}</h4>
            ...
            <div class="mt-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <div class="grid grid-cols-4 gap-2">
                    @foreach(['S', 'M', 'L', 'XL'] as $size)
                        <button class="h-10 border border-gray-300 rounded-md flex items-center justify-center text-black hover:bg-gray-100 transition text-xs">{{ $size }}</button>
                    @endforeach
                </div>
            </div>
        </div>
        @empty
        <div class="col-span-full text-center py-20"><p class="text-gray-500 text-lg">No women's socks on sale.</p></div>
        @endforelse
    </section>

    {{-- Bottom Categories --}}
    <section class="w-full px-6 py-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        @foreach([['title' => 'Shoes', 'img' => 'first.webp'], ['title' => 'Apparel', 'img' => 'second.webp'], ['title' => 'Accessories', 'img' => 'third.webp']] as $cat)
        <div class="relative rounded-2xl overflow-hidden shadow-lg group">
            <img src="{{ asset('images/' . $cat['img']) }}" class="w-full h-[350px] object-cover transition duration-700 group-hover:scale-105" />
            <h1 class="absolute inset-0 flex items-center justify-center text-white text-4xl bg-black/10 font-serif">{{ $cat['title'] }}</h1>
        </div>
        @endforeach
    </section>
</div>
