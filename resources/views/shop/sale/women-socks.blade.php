...
<div class="relative z-50 w-full max-w-6xl mx-auto px-6 py-4 flex items-center justify-between bg-[#E6DDD0] rounded-[40px] mt-4">
    ...
    <div class="flex items-center gap-4">
        <div class="flex items-center bg-[#E5DFD6] border border-[#D4CDC3] rounded-full p-[4px]">
            <a href="{{ url('/sale/men/socks') }}"
               class="px-6 py-1.5 rounded-full text-[13px] font-medium transition
               {{ request()->is('sale/men/socks*') ? 'bg-black text-white' : 'text-black' }}">MEN</a>
            <a href="{{ url('/sale/women/socks') }}"
               class="px-6 py-1.5 rounded-full text-[13px] font-medium transition
               {{ request()->is('sale/women/socks*') ? 'bg-black text-white' : 'text-black' }}">WOMEN</a>
        </div>
...
<x-product-grid>
    @forelse($products as $product)
        <x-product-card
            ...
            :subtitle="$product->description ?? 'Women\'s Socks'"
    ...
    @php
        $sockSizes = ['S' => 'W5-7', 'M' => 'W8-10', 'L' => 'W11/M9-12', 'XL' => 'M13-14'];
    @endphp
    @foreach($sockSizes as $size => $range)
        @if($product->sizes && isset($product->sizes[$size]) && $product->sizes[$size] <= 0)
            <div class="relative h-14 border border-gray-200 rounded-lg flex flex-col items-center justify-center text-gray-300 text-xs overflow-hidden">
    ...
    @empty
        <div class="col-span-full text-center py-20 text-gray-400">
            <p class="text-lg">No sale socks found.</p>
        </div>
    @endforelse
</x-product-grid>
