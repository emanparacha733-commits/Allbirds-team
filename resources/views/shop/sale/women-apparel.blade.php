...
<section class="relative w-full h-[50vh] md:h-[60vh] bg-cover bg-center rounded-2xl" style="background-image: url('/images/hero1.jpeg');">
    <div class="relative z-10 max-w-7xl mx-auto h-full flex flex-col justify-between md:px-8 text-white">
        <div class="flex gap-6 text-sm font-light font-serif tracking-wide mt-12">
            ...
            <a href="/sale" class="hover:underline">Sale /</a>
    ...
    <div class="flex items-center gap-4">
        <div class="flex items-center bg-[#E5DFD6] border border-[#D4CDC3] rounded-full p-[4px]">
            <a href="{{ url('/sale/men/apparel') }}"
               ...
               {{ request()->is('sale/men/apparel*') ? 'bg-black text-white' : 'text-black' }}">MEN</a>
            <a href="{{ url('/sale/women/apparel') }}"
               class="px-6 py-1.5 rounded-full text-[13px] font-medium transition
               {{ request()->is('sale/women/apparel*') ? 'bg-black text-white' : 'text-black' }}">WOMEN</a>
        </div>
        <div class="relative inline-block">
            <button id="sortBtn"
                ...
                {{ request('sort') === 'price_low' ? 'PRICE, LOW TO HIGH' : (request('sort') === 'price_high' ? 'PRICE, HIGH TO LOW' : (request('sort') === 'alpha_asc' ? 'ALPHABETICALLY, A-Z' : (request('sort') === 'alpha_desc' ? 'ALPHABETICALLY, Z-A' : 'BEST SELLING'))) }}
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transition-transform duration-200" id="arrowIcon" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
            </button>
            <div id="dropdownMenu" class="absolute right-0 mt-2 w-52 bg-black rounded-xl shadow-2xl hidden z-[9999]">
...
<x-product-grid>
    @forelse($products as $product)
        <x-product-card
            ...
            :subtitle="$product->description ?? 'Women\'s Apparel'"
    ...
    @php $apparelSizes = ['XS', 'S', 'M', 'L', 'XL', 'XXL']; @endphp
    @foreach($apparelSizes as $size)
        @if($product->sizes && isset($product->sizes[$size]) && $product->sizes[$size] <= 0)
            <div class="relative h-10 border border-gray-200 rounded-lg flex items-center justify-center text-gray-300 text-xs overflow-hidden">
                <span class="font-medium">{{ $size }}</span>
                <svg class="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
                    <line x1="10%" y1="90%" x2="90%" y2="10%" stroke="#D1D5DB" stroke-width="1"/>
                </svg>
            </div>
        @else
            <button class="h-10 border border-gray-200 rounded-lg flex items-center justify-center text-black hover:border-black transition text-xs font-medium">
    ...
    @empty
        <div class="col-span-full text-center py-20 text-gray-400">
            <p class="text-lg">No sale apparel found.</p>
        </div>
    @endforelse
</x-product-grid>

{{-- Bottom Category Cards --}}
<section class="relative z-10 w-full px-2 sm:px-4 lg:px-6 py-6 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4 md:gap-6 lg:gap-4">
    <div class="relative rounded-2xl overflow-hidden shadow-lg w-full mx-auto cursor-pointer group">
        <img src="{{ asset('images/first.webp') }}" class="w-full h-[400px] sm:h-[300px] md:h-[350px] lg:h-auto object-cover rounded-2xl transition-transform duration-1000 ease-out group-hover:scale-105" />
        ...
        <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-4">
            <a href="{{ url('/sale/men/shoes') }}" class="bg-transparent border border-white text-white rounded-2xl shadow-lg flex items-center justify-center w-40 h-[5vh] hover:bg-white hover:text-black transition">Shop Men</a>
            <a href="{{ url('/sale/women/shoes') }}" class="bg-transparent border border-white text-white rounded-2xl shadow-lg flex items-center justify-center w-40 h-[5vh] hover:bg-white hover:text-black transition">Shop Women</a>
        </div>
    </div>
    <div class="relative rounded-2xl overflow-hidden shadow-lg w-full mx-auto cursor-pointer group">
        <img src="{{ asset('images/second.webp') }}" class="w-full h-[400px] sm:h-[300px] md:h-[350px] lg:h-auto object-cover rounded-2xl transition-transform duration-1000 ease-out group-hover:scale-105" />
        ...
        <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-4">
            <a href="{{ url('/sale/men/apparel') }}" class="bg-transparent border border-white text-white rounded-2xl shadow-lg flex items-center justify-center w-40 h-[5vh] hover:bg-white hover:text-black transition">Shop Men</a>
            <a href="{{ url('/sale/women/apparel') }}" class="bg-transparent border border-white text-white rounded-2xl shadow-lg flex items-center justify-center w-40 h-[5vh] hover:bg-white hover:text-black transition">Shop Women</a>
        </div>
    </div>
    <div class="relative rounded-2xl overflow-hidden shadow-lg w-full mx-auto cursor-pointer group">
        <img src="{{ asset('images/third.webp') }}" class="w-full h-[400px] sm:h-[300px] md:h-[350px] lg:h-auto object-cover rounded-2xl transition-transform duration-1000 ease-out group-hover:scale-105" />
        <h1 class="absolute inset-0 flex items-center justify-center text-white text-4xl font-light font-serif bg-black/10 pointer-events-none">Socks</h1>
        <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-4">
            <a href="{{ url('/sale/men/socks') }}" class="bg-transparent border border-white text-white rounded-2xl shadow-lg flex items-center justify-center w-40 h-[5vh] hover:bg-white hover:text-black transition">Shop Men</a>
            <a href="{{ url('/sale/women/socks') }}" class="bg-transparent border border-white text-white rounded-2xl shadow-lg flex items-center justify-center w-40 h-[5vh] hover:bg-white hover:text-black transition">Shop Women</a>
        </div>
    </div>
</section>
