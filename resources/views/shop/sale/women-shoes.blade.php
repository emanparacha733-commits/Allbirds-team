    ...
    <div class="flex items-center gap-4">
        <div class="flex items-center bg-[#E5DFD6] border border-[#D4CDC3] rounded-full p-[4px]">
            <a href="{{ url('/sale/men/shoes') }}"
               ...
               {{ request()->is('sale/men/shoes*') ? 'bg-black text-white' : 'text-black' }}">MEN</a>
            <a href="{{ url('/sale/women/shoes') }}"
               class="px-6 py-1.5 rounded-full text-[13px] font-medium transition
               {{ request()->is('sale/women/shoes*') ? 'bg-black text-white' : 'text-black' }}">WOMEN</a>
        </div>
...
<x-product-grid>
    @forelse($products as $product)
        <x-product-card
            ...
            :subtitle="$product->description ?? 'Women\'s Shoes'"
            ...
            <div class="grid grid-cols-5 gap-2">
                @forelse($product->available_sizes as $size)
                    <div class="h-10 border border-gray-300 rounded-md flex items-center justify-center hover:bg-gray-200 text-sm cursor-pointer">{{ $size }}</div>
                @empty
    ...
    @empty
        <div class="col-span-full text-center py-20 text-gray-400">
            <p class="text-lg">No sale shoes found.</p>
        </div>
    @endforelse
</x-product-grid>
...
<section class="max-w-full px-6 py-8 mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <div class="bg-white rounded-2xl shadow-xl p-6 w-full flex flex-col gap-4 justify-start">
        ...
        <p class="text-gray-600 text-sm">We replace petroleum-based synthetics with natural alternatives wherever we can. Like using wool, tree fiber, and sugarcane. They're soft, breathable, and better for the planet.</p>
    </div>
</section>
