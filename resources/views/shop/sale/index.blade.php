...
<div class="bg-[#f5f2ed] min-h-screen overflow-visible">
    ...
    <section class="w-full pt-6 pb-10 px-4 text-center">
        ...
        <h1 class="text-3xl font-serif font-light text-black">Sale</h1>
        <p class="mt-3 text-sm text-gray-600 max-w-lg mx-auto leading-relaxed">
            Shop our best deals on shoes, apparel, and accessories. Premium materials, natural comfort — now at a better price.
        </p>
    </section>
        ...
        <div class="flex items-center gap-4">
            ...
            <div class="flex items-center bg-[#E5DFD6] border border-[#D4CDC3] rounded-full p-[4px]">
                <a href="{{ url('/sale') }}"
                   ...
                   {{ request()->is('sale') ? 'bg-black text-white' : 'text-black' }}">
                   ALL
                </a>
                <a href="{{ url('/sale/shoes') }}"
                   class="px-5 py-1.5 rounded-full text-[13px] font-medium transition
                   {{ request()->is('sale/shoes*') ? 'bg-black text-white' : 'text-black' }}">
                   SHOES
                </a>
                <a href="{{ url('/sale/apparel') }}"
                   class="px-5 py-1.5 rounded-full text-[13px] font-medium transition
                   {{ request()->is('sale/apparel*') ? 'bg-black text-white' : 'text-black' }}">
                   APPAREL
                </a>
                <a href="{{ url('/sale/socks') }}"
                   class="px-5 py-1.5 rounded-full text-[13px] font-medium transition
                   {{ request()->is('sale/socks*') ? 'bg-black text-white' : 'text-black' }}">
                   SOCKS
                </a>
            </div>
    ...
    <div class="relative max-w-full px-4 pt-6 pb-32 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-6 items-start overflow-visible">
        ...
        @empty
            <div class="col-span-full text-center py-20">
                <p class="text-gray-500 text-lg">No sale products found.</p>
                <a href="{{ url('/') }}"
    ...
    <section class="max-w-full px-6 py-8 mx-auto grid
        ...
        <div class="bg-white rounded-2xl shadow-xl p-6 w-full flex flex-col gap-4">
            <p class="text-xl text-black">Real Savings, Real Comfort</p>
            <p class="text-gray-600 text-sm">Get the same premium Allbirds comfort you love, now at reduced prices. Stock is limited — grab your favorites before they're gone.</p>
        </div>
        <div class="bg-white rounded-2xl shadow-xl p-6 w-full flex flex-col gap-4">
